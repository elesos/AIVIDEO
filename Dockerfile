...
RUN chmod 777 /AIVIDEO

ENV PYTHONPATH="/AIVIDEO"
...
## For Linux or MacOS:
# docker run -v $(pwd)/config.toml:/AIVIDEO/config.toml -v $(pwd)/storage:/AIVIDEO/storage -p 127.0.0.1:8501:8501 moneyprinterturbo
## For Windows:
# docker run -v ${PWD}/config.toml:/AIVIDEO/config.toml -v ${PWD}/storage:/AIVIDEO/storage -p 127.0.0.1:8501:8501 moneyprinterturbo
